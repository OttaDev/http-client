...
export { OttaHttpClientTypes as OttaDevHttpTypes } from './definitions';

type ENVIRONMENT = 'node' | 'browser';

export class OttaHttpClient {
  private _environment: ENVIRONMENT = ((typeof process !== 'undefined') && (process.release.name === 'node')) ? 'node' : 'browser';
  ...
  constructor(settings?: OttaHttpClientTypes.Settings) {
    if (settings) {
      ...
      if (settings.history && !isNaN(settings.history) && settings.history >= 0) {
   ...
   */
  public request(request: OttaHttpClientTypes.Request, replaceHeaders = false): Promise<OttaHttpClientTypes.Response> {
    ...
    const parsedRequest: OttaHttpClientTypes.Request = { body, headers, method, url };

    return (this._environment === 'node' ? nodeRequest : browserRequest)(parsedRequest)
